        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-9 order-lg-2">
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-xl-4 col-md-4 col-sm-6  mt--10 mt-sm--0">
                                <span class="toolbar-status">
                                    Showing {{ $new_arrivals_products->firstItem() }} to
                                    {{ $new_arrivals_products->lastItem() }} of
                                    {{ $new_arrivals_products->total() }}
                                    ({{ $new_arrivals_products->lastPage() }} Pages)
                                </span>
                            </div>
                                    ...
                                    <div class="product-grid-content">
                                        ...
                                        <div class="product-card--body">
                                            <div class="card-image">
                                                @php
                                                    $product_image = json_decode($new_arrivals_product->image);
                                                @endphp
                                                ...
                                                <div class="hover-contents">
                                                    <a href="{{ route('botu.product.details', [$new_arrivals_product->id, $new_arrivals_product->slug]) }}"
                                                        class="hover-image">
                                                        @if (!empty($product_image[0]))
                                                            <img src="{{ url('/uploads/product_images/' . $product_image[0]) }}"
                                                                height="220" width="220" alt="Product Image">
                                                    ...
                                                    <div class="hover-btns">
                                                        <form
                                                            action="{{ route('botu.add_to_cart', $new_arrivals_product->id) }}"
                                                            method="post">
                                                            @csrf
                                                        ...
                                                        <form
                                                            action="{{ route('botu.add_to_wishlist', $new_arrivals_product->id) }}"
                                                            method="post">
                                                            @csrf
                                                            <button type="submit" class="single-btn">
                                                        ...
                                                        <a data-toggle="modal" class="single-btn product_details_modal"
                                                            data-id="{{ $new_arrivals_product->id }}"
                                                            data-name="{{ $new_arrivals_product->name }}"
                                                            data-author="{{ $new_arrivals_product->author }}"
                                                            data-image="{{ $new_arrivals_product->image }}"
                                                            data-stock-status="{{ $new_arrivals_product->stock_status }}"
                                                            data-price="{{ $new_arrivals_product->price }}"
                                                            data-discount="{{ $new_arrivals_product->discount }}"
                                                            data-short-description="{{ $new_arrivals_product->short_description }}">
                                            ...
                                            <div class="price-block">
                                                ...
                                                <del class="price-old">৳{{ $new_arrivals_product->price }}</del>
                                                <span class="price-discount">{{ $new_arrivals_product->discount }}%</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    @if ($new_arrivals_products->isEmpty())
