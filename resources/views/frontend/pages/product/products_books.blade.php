            ...
            <div class="breadcrumb-contents">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">Products Books</li>
                    </ol>
                </nav>
            </div>
        </div>
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-xl-4 col-md-4 col-sm-6  mt--10 mt-sm--0">
                                <span class="toolbar-status">
                                    Showing {{ $all_books->firstItem() }} to
                                    {{ $all_books->lastItem() }} of
                                    {{ $all_books->total() }}
                                    ({{ $all_books->lastPage() }} Pages)
                                </span>
                            </div>
                                ...
                                <div class="product-card">
                                    <div class="product-grid-content">
                                        <div class="product-header">
                                            <a href="{{ route('botu.author.instructor', $book->author) }}" class="author">
                                                {{ $book->author }}
                                            </a>
                                            <h3>
                                                <a
                                                    href="{{ route('botu.product.details', [$book->id, $book->slug]) }}">{{ $book->name }}</a>
                                            </h3>
                                        </div>
                                        <div class="product-card--body">
                                            <div class="card-image">
                                                @php
                                                    $book_image = json_decode($book->image);
                                                @endphp
                                                @if (!empty($book_image[0]))
                                                    <img src="{{ url('/uploads/product_images/' . $book_image[0]) }}"
                                                ...
                                                <div class="hover-contents">
                                                    <a href="{{ route('botu.product.details', [$book->id, $book->slug]) }}"
                                                        class="hover-image">
                                                        @if (!empty($book_image[0]))
                                                            <img src="{{ url('/uploads/product_images/' . $book_image[0]) }}"
                                                                height="220" width="220" alt="Product Image">
                                                    ...
                                                    <div class="hover-btns">
                                                        <form action="{{ route('botu.add_to_cart', $book->id) }}"
                                                            method="post">
                                                            @csrf
                                                        ...
                                                        <form action="{{ route('botu.add_to_wishlist', $book->id) }}"
                                                            method="post">
                                                            @csrf
                                                            <button type="submit" class="single-btn">
                                                        ...
                                                        <a data-toggle="modal" class="single-btn product_details_modal"
                                                            ...
                                                            data-stock-status="{{ $book->stock_status }}"
                                                            data-price="{{ $book->price }}"
                                                            data-discount="{{ $book->discount }}"
                                                            data-short-description="{{ $book->short_description }}">
                                            ...
                                            <div class="price-block">
                                                <span
                                                    class="price">৳{{ $book->price - ($book->price * $book->discount) / 100 }}</span>
