            ...
            <div class="breadcrumb-contents">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">Products Lists Tutorials</li>
                    </ol>
                </nav>
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-lg-9 order-lg-2">
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-xl-4 col-md-4 col-sm-6  mt--10 mt-sm--0">
                                <span class="toolbar-status">
                                    Showing {{ $tutorials_category_products->firstItem() }} to
                                    {{ $tutorials_category_products->lastItem() }} of
                                    {{ $tutorials_category_products->total() }}
                                    ({{ $tutorials_category_products->lastPage() }} Pages)
                                </span>
                            </div>
                    ...
                    <div class="shop-product-wrap grid with-pagination row space-db--30 shop-border">
                        @foreach ($tutorials_category_products as $tutorials_category_product)
                            <div class="col-lg-4 col-sm-6">
                                <div class="product-card">
                                    <div class="product-grid-content">
                                        <div class="product-header">
                                            <a href="{{ route('botu.author.instructor', $tutorials_category_product->author) }}"
                                                class="author">
                                                {{ $tutorials_category_product->author }}
                                            </a>
                                            <h3>
                                                <a
                                                    href="{{ route('botu.product.details', [$tutorials_category_product->id, $tutorials_category_product->slug]) }}">{{ $tutorials_category_product->name }}</a>
                                            </h3>
                                        </div>
                                        <div class="product-card--body">
                                            <div class="card-image">
                                                @php
                                                    $cat_wise_image = json_decode($tutorials_category_product->image);
                                                @endphp
                                                ...
                                                <div class="hover-contents">
                                                    <a href="{{ route('botu.product.details', [$tutorials_category_product->id, $tutorials_category_product->slug]) }}"
                                                        class="hover-image">
                                                        @if (!empty($cat_wise_image[0]))
                                                            <img src="{{ url('/uploads/product_images/' . $cat_wise_image[0]) }}"
                                                                height="220" width="220" alt="Product Image">
                                                    ...
                                                    <div class="hover-btns">
                                                        <form
                                                            action="{{ route('botu.add_to_cart', $tutorials_category_product->id) }}"
                                                            method="post">
                                                            @csrf
                                                        ...
                                                        <form
                                                            action="{{ route('botu.add_to_wishlist', $tutorials_category_product->id) }}"
                                                            method="post">
                                                            @csrf
                                                            <button type="submit" class="single-btn">
                                                        ...
                                                        <a data-toggle="modal" class="single-btn product_details_modal"
                                                            data-id="{{ $tutorials_category_product->id }}"
                                                            data-name="{{ $tutorials_category_product->name }}"
                                                            data-author="{{ $tutorials_category_product->author }}"
                                                            data-image="{{ $tutorials_category_product->image }}"
                                                            data-stock-status="{{ $tutorials_category_product->stock_status }}"
                                                            data-price="{{ $tutorials_category_product->price }}"
                                                            data-discount="{{ $tutorials_category_product->discount }}"
                                                            data-short-description="{{ $tutorials_category_product->short_description }}">
                                            ...
                                            <div class="price-block">
                                                <span
                                                    class="price">৳{{ $tutorials_category_product->price - ($tutorials_category_product->price * $tutorials_category_product->discount) / 100 }}</span>
                                                <del class="price-old">৳{{ $tutorials_category_product->price }}</del>
                                                <span
                                                    class="price-discount">{{ $tutorials_category_product->discount }}%</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    @if ($tutorials_category_products->isEmpty())
                    ...
                    {{-- Pagination Block --}}
                    <br><br>
                    {!! $tutorials_category_products->links() !!}
