                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-xl-4 col-md-4 col-sm-6  mt--10 mt-sm--0">
                                <span class="toolbar-status">
                                    Showing {{ $category_products->firstItem() }} to {{ $category_products->lastItem() }} of
                                    {{ $category_products->total() }}
                                    ({{ $category_products->lastPage() }} Pages)
                                </span>
                            </div>
                                ...
                                <div class="product-card">
                                    <div class="product-grid-content">
                                        <div class="product-header">
                                            <a href="{{ route('botu.author.instructor', $category_product->author) }}" class="author">
                                                {{ $category_product->author }}
                                            </a>
                                            <h3>
                                                @if ($category_product->type == 'Book')
                                                    <a
                                                        href="{{ route('botu.product.details', [$category_product->id, $category_product->slug]) }}">{{ $category_product->name }}</a>
                                                @else
                                                    <a
                                                        href="{{ route('botu.tutorial.details', [$category_product->id, $category_product->slug]) }}">{{ $category_product->name }}</a>
                                                @endif
                                            </h3>
                                        </div>
                                        <div class="product-card--body">
                                            <div class="card-image">
                                                @php
                                                    $cat_wise_image = json_decode($category_product->image);
                                                @endphp
                                                ...
                                                <div class="hover-contents">
                                                    @if ($category_product->type == 'Book')
                                                        <a href="{{ route('botu.product.details', [$category_product->id, $category_product->slug]) }}"
                                                            class="hover-image">
                                                            @if (!empty($cat_wise_image[0]))
                                                                <img src="{{ url('/uploads/product_images/' . $cat_wise_image[0]) }}"
                                                                    height="220" width="220" alt="Product Image">
                                                    ...
                                                    @else
                                                        <a href="{{ route('botu.tutorial.details', [$category_product->id, $category_product->slug]) }}"
                                                            class="hover-image">
                                                            @if (!empty($cat_wise_image[0]))
                                                                <img src="{{ url('/uploads/product_images/' . $cat_wise_image[0]) }}"
                                                                    height="220" width="220" alt="Product Image">
                                                    ...
                                                    <div class="hover-btns">
                                                        <form
                                                            action="{{ route('botu.add_to_cart', $category_product->id) }}"
                                                            method="post">
                                                            @csrf
                                                        ...
                                                        <form
                                                            action="{{ route('botu.add_to_wishlist', $category_product->id) }}"
                                                            method="post">
                                                            @csrf
                                                            <button type="submit" class="single-btn">
                                                        ...
                                                        <a data-toggle="modal" class="single-btn product_details_modal"
                                                            ...
                                                            data-author="{{ $category_product->author }}"
                                                            data-image="{{ $category_product->image }}"
                                                            data-stock-status="{{ $category_product->stock_status }}"
                                                            data-price="{{ $category_product->price }}"
                                                            data-discount="{{ $category_product->discount }}"
                                                            data-short-description="{{ $category_product->short_description }}">
                                            ...
                                            <div class="price-block">
                                                <span
                                                    class="price">৳{{ $category_product->price - ($category_product->price * $category_product->discount) / 100 }}</span>
                                                <del class="price-old">৳{{ $category_product->price }}</del>
                                                <span class="price-discount">{{ $category_product->discount }}%</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
