                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-xl-4 col-md-4 col-sm-6  mt--10 mt-sm--0">
                                <span class="toolbar-status">
                                    Showing {{ $all_tutorials->firstItem() }} to {{ $all_tutorials->lastItem() }} of
                                    {{ $all_tutorials->total() }}
                                    ({{ $all_tutorials->lastPage() }} Pages)
                                </span>
                            </div>
                                ...
                                <div class="product-card">
                                    <div class="product-grid-content">
                                        <div class="product-header">
                                            <a href="{{ route('botu.author.instructor', $tutorial->author) }}"
                                                class="author">
                                                {{ $tutorial->author }}
                                            </a>
                                            <h3>
                                                <a
                                                    href="{{ route('botu.tutorial.details', [$tutorial->id, $tutorial->slug]) }}">{{ $tutorial->name }}</a>
                                            </h3>
                                        </div>
                                        <div class="product-card--body">
                                            <div class="card-image">
                                                @php
                                                    $tutorial_image = json_decode($tutorial->image);
                                                @endphp
                                                @if (!empty($tutorial_image[0]))
                                                    <img src="{{ url('/uploads/product_images/' . $tutorial_image[0]) }}"
                                                ...
                                                <div class="hover-contents">
                                                    <a href="{{ route('botu.tutorial.details', [$tutorial->id, $tutorial->slug]) }}"
                                                        class="hover-image">
                                                        @if (!empty($tutorial_image[0]))
                                                            <img src="{{ url('/uploads/product_images/' . $tutorial_image[0]) }}"
                                                                height="220" width="220" alt="Product Image">
                                                    ...
                                                    <div class="hover-btns">
                                                        <form action="{{ route('botu.add_to_cart', $tutorial->id) }}"
                                                            method="post">
                                                            @csrf
                                                        ...
                                                        <form action="{{ route('botu.add_to_wishlist', $tutorial->id) }}"
                                                            method="post">
                                                            @csrf
                                                            <button type="submit" class="single-btn">
                                                        ...
                                                        <a data-toggle="modal" class="single-btn product_details_modal"
                                                            data-id="{{ $tutorial->id }}"
                                                            data-name="{{ $tutorial->name }}"
                                                            data-author="{{ $tutorial->author }}"
                                                            data-image="{{ $tutorial->image }}"
                                                            data-stock-status="{{ $tutorial->stock_status }}"
                                                            data-price="{{ $tutorial->price }}"
                                                            data-discount="{{ $tutorial->discount }}"
                                                            data-short-description="{{ $tutorial->short_description }}">
                                            ...
                                            <div class="price-block">
                                                <span
                                                    class="price">৳{{ $tutorial->price - ($tutorial->price * $tutorial->discount) / 100 }}</span>
                                                <del class="price-old">৳{{ $tutorial->price }}</del>
                                                <span class="price-discount">{{ $tutorial->discount }}%</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
